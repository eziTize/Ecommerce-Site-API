            ...
            <div class="container">
                <div class="inner-title-list">
                    <ul>
                        ...
                        <li><router-link to="/all-products">{{ site_lang['PRODUCTS'] }}</router-link> <i class="fas fa-angle-right" aria-hidden="true"></i> </li>
                                    ...
                                    <div class="panel-group" id="accordion" role="tablist" aria-multiselectable="true">
                                        ...
                                        <div class="panel panel-default" v-for="product_menu in products_menu">
                                            <div class="panel-heading" :id="'heading-'+product_menu.id" role="tab">
                                                <h4 class="panel-title">
                                                    <a :class="(product_menu.id == selected_menu) ? '' : 'collapsed'" role="button" data-toggle="collapse" data-parent="#accordion" :href="'#collapse-'+product_menu.id" :aria-expanded="(product_menu.id == selected_menu) ? true : false" aria-controls="collapseOne">{{ product_menu.lang_value }}<i class="pull-right fa fa-plus"></i></a>
                                                </h4>
                                            </div>
                                            <div :class="(product_menu.id == selected_menu) ? 'panel-collapse collapse in' : 'panel-collapse collapse'" :id="'collapse-'+product_menu.id" role="tabpanel" :aria-labelledby="'heading-'+product_menu.id">
                                                <div class="panel-body pxlr-faq-body">
                                                    <ul>
    ...
    export default {
        ...
        methods: {
            handleSearch(e) {
                ...
                if(this.search){
                    this.$router.push((nextUrl != null ? nextUrl : '/products/'+this.$route.params.slug+'?search='+this.search))
                }else{
                    this.$router.push((nextUrl != null ? nextUrl : '/products/'+this.$route.params.slug))
                }
            },
            fetchData() {
                let language = this.language
                axios.post(this.apiRootUrl+'/products/'+this.$route.params.slug+'?search='+this.search, {
                    language,
                }).then(response => {
